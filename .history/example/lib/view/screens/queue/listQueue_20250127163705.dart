    ...
    try {
      ...
      if (res.code == "200" && context.mounted) {
          ...
          queueList1 = List<Queue>.from(
            data.where((item) => (item['number'].equals())).map((jsonItem) => Queue.fromJson(jsonItem))
          );
            queueList2 = List<Queue>.from(
            data.where((item) => (item['id']%3 == 1)).map((jsonItem) => Queue.fromJson(jsonItem))
          );
            queueList3 = List<Queue>.from(
            data.where((item) => (item['id']%3 == 2)).map((jsonItem) => Queue.fromJson(jsonItem))
          );
          setState(() {
           _isLoading = false;
          });
