    ...
    try {
      ...
      if (res.code == "200" && context.mounted) {
          ...
          queueList1 = List<Queue>.from(
            data.where((item) => (item['number'].equal('ONE'))).map((jsonItem) => Queue.fromJson(jsonItem))
          );
            queueList2 = List<Queue>.from(
            data.where((item) => (item['id'].equal('TWO'))).map((jsonItem) => Queue.fromJson(jsonItem))
          );
            queueList3 = List<Queue>.from(
            data.where((item) => (item['id'].equal('THREE'))).map((jsonItem) => Queue.fromJson(jsonItem))
          );
          setState(() {
           _isLoading = false;
          });
