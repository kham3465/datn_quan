    ...
    try {
      ...
      if (res.code == "200" && context.mounted) {
          ...
          queueList1 = List<Queue>.from(
            data.where((item) => (item['number'].toString()=='ONE')).map((jsonItem) => Queue.fromJson(jsonItem))
          );
            queueList2 = List<Queue>.from(
            data.where((item) => (item['id'].toString()=='TWO')).map((jsonItem) => Queue.fromJson(jsonItem))
          );
