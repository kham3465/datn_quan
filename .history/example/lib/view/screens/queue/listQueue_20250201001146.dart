...
class ListQueue extends StatefulWidget {
  ...
  const ListQueue({super.key, required this.idVehicle, required this.number});
...
class _ListQueueState extends State<ListQueue>
  ...
  Future<void> getDataListQueue(int idVehicle, String number) async {
    try {
      final res = await ApiRequest.getListQueue(idVehicle, number);
      if (res.code == "200" && context.mounted) {
          ...
          queueList1 = List<Queue>.from(
            ...
            data.where((item) => (item['status'].toString()=='DONE')).map((jsonItem) => Queue.fromJson(jsonItem))
          );
            queueList3 = List<Queue>.from(
            data.where((item) => (item['status'].toString()=='FALSE')).map((jsonItem) => Queue.fromJson(jsonItem))
          );
          setState(() {
           _isLoading = false;
          });
  ...
  void initState() {
    ...
    getDataListQueue(widget.idVehicle, widget.number);
  }
  @override
  void dispose() {
