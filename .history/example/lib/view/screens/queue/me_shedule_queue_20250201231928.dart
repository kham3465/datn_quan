...
class _MeQueueState extends State<MeQueue> {
  ...
  Future<void> cancelQueue(String status) async {
    ...
    try {
      final res = await ApiRequest.updateQueueMe(id, sta);
  ...
  void showCancelConfirmationDialog() {
   ...
   void showDoneConfirmationDialog() {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text("Xác nhận hoàn thành"),
        content: Text("Bạn bấm xác nhận nếu đã hoàn thành"),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(),
            child: Text("Đồng ý"),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: queue == null
          ? Center(child: Text("Không có dữ liệu nào hoặc lịch đang chờ"))
          :Padding(
