    ...
    try {
      ...
      if (res.code == "200" && context.mounted) {
          ...
          queueList1 = List<Queue>.from(
            data.where((item) => (item['status'].toString()=='ONE')).map((jsonItem) => Queue.fromJson(jsonItem))
          );
            queueList2 = List<Queue>.from(
            data.where((item) => (item['status'].toString()=='TWO')).map((jsonItem) => Queue.fromJson(jsonItem))
          );
            queueList3 = List<Queue>.from(
            data.where((item) => (item['status'].toString()=='THREE')).map((jsonItem) => Queue.fromJson(jsonItem))
          );
          setState(() {
           _isLoading = false;
          });
