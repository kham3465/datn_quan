...
class ApiRequest {
  static const String domain = "http://37.120.222.8:3541/api/v1";
  ...
  static Future<ApiResponse> createQueue(String idUser, String idVehicle) async {
    Map data = {"name": idUser, "idVehicle": idVehicle};
    return await ApiClient().request(
        url: "$domain/auth/authenticate",
        data: json.encode(data),
        method: ApiClient.post);
  ...
  static Future<ApiResponse> getListQueue(
    ...
    return await ApiClient().request(
        url:
            "$domain/public/get_list_queue?status=${status}&district=${district}",
        // data: json.encode(data),
        method: ApiClient.post);
  }
