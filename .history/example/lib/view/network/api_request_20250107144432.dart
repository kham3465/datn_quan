...
class ApiRequest {
  ...
  static Future<ApiResponse> getListShip(
    ) async {
    return await ApiClient().request(
...
String name,String nationaly,String typeShip
    ) async {
    return await ApiClient().request(
        url: "$domain/admin/ship/list?name=${name}&typeShip=${typeShip}&nationaly=${nationaly}",
        // data: json.encode(data),
        method: ApiClient.post);
  }
     static Future<ApiResponse> createLocation(
String longitude,String latitude,int idShip
    ) async {
...
int idShip,DateTime createTime, DateTime endTime
    ) async {
    
    return await ApiClient().request(
        url: "$domain/public/location?idShip=${idShip}&startTime=${createTime.toIso8601String()}&endTime=${endTime.toIso8601String()}",
        // data: json.encode(data),
        method: ApiClient.get);
  }
    static Future<ApiResponse> delete(int id
    ) async {
    return await ApiClient().request(
  ...
  //register
  static Future<ApiResponse> userRegister(String name,
   String phone,
   String password,
 String email,
 DateTime dob,
...
static Future<ApiResponse> update({
  ...
  static Future<ApiResponse> getListUserService() async {
    return await ApiClient()
        .request(url: "$domain/admin/user/list?role=SERVICE", method: ApiClient.get);
  }
    static Future<ApiResponse> getListUserSailor() async {
    return await ApiClient()
        .request(url: "$domain/admin/user/list?role=SAILOR", method: ApiClient.get);
  }
     static Future<ApiResponse> deleteUser(int id) async {
    return await ApiClient()
        .request(url: "$domain/admin/user/delete/${id}", method: ApiClient.delete);
  }
  //upload-image
