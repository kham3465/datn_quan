...
class MapNew extends StatefulWidget {
  ...
  @override
  State createState() => MapNewState();
}

class MapNewState extends State<MapNew>
...
Future<void> getLocation() async {
  // Thiết lập Timer để gọi API mỗi 3 giây
  Timer.periodic(Duration(seconds: 3), (timer) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    final int? idUser = prefs.getInt('idUser');
    final res = await ApiRequest.getLocationNew(idUser ?? 1);
    ...
    if (res.code == "200") {
      ...
      // Đảo ngược thứ tự các điểm trong danh sách
      ...
      setState(() {
        // Cập nhật Marker với các điểm tọa độ sau khi đảo ngược thứ tự
        _polygonGeofenceCursorMarkers = newList.asMap().entries.map((entry) {
          int index = entry.key;
          LatLng point = entry.value;

          // Nếu là điểm đầu tiên (sau khi đảo ngược), sử dụng biểu tượng location màu đỏ
          if (index == 0) {
            return Marker(
              width: 40.0,
              height: 40.0,
            ...
            return Marker(
              ...
              builder: (ctx) => Transform.rotate(
                angle: -angle, // Đảo ngược góc để mũi tên chỉ đúng hướng
                child: Icon(
        ...
        // Di chuyển bản đồ đến điểm cuối cùng (sau khi đảo ngược)
