...
class HomeMap extends StatefulWidget {
  ...
  @override
  State createState() => HomeMapState();
}

class HomeMapState extends State<HomeMap>
    with AutomaticKeepAliveClientMixin<HomeMap> {
  static const LOCATION_ARROW_IMAGE_PATH =
      "assets/images/markers/location-arrow-blue.png";
  List<LatLng> list = [];

  Future<void> getLocation(String district, String name) async {
  final res = await ApiRequest.getListElectric(district, name);
  if (res.code == "200") {
    print("Thành công");
    List data = res.data;
    
    // Chuyển đổi dữ liệu thành danh sách các ElectricVehicle
    ...
    setState(() {
      _polygonGeofenceCursorMarkers = vehicles.map((vehicle) {
        LatLng point = LatLng(double.parse(vehicle.latitude), double.parse(vehicle.longitude));
        ...
        return Marker(
          ...
          builder: (ctx) {
            return GestureDetector(
              onTap: () {
                // Khi nhấn vào marker, gọi hàm để hiện dialog với thông tin từ vehicle
                _showMarkerDialog(context, vehicle);
              },
              child: Icon(
                Icons.local_gas_station, // Biểu tượng cây xăng
                color: const Color.fromARGB(255, 0, 0, 0), // Màu đen cho cây xăng
                size: 30.0,
              ),
            );
          },
      ...
      _moveToFirstPoint(vehicles.map((vehicle) {
        return LatLng(double.parse(vehicle.latitude), double.parse(vehicle.longitude));
      }).toList());
    });
  } else {
    print("Thất bại");
...
void _showMarkerDialog(BuildContext context, ElectricVehicle vehicle) {
  ...
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        title: Text("Cây xăng: ${vehicle.name}"), // Tiêu đề là tên phương tiện
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text("Quận: ${vehicle.district}"),  // Hiển thị quận
            SizedBox(height: 8), // Khoảng cách giữa các dòng
            Text("Trạng thái: $status"), // Hiển thị trạng thái
          ],
        ),
        actions: <Widget>[
          // Nút "Đăng ký" sẽ bị vô hiệu hóa nếu isBusy = true
          TextButton(
            onPressed: isBusy ? null : () {
              // Thực hiện hành động đăng ký nếu isBusy là false
              print("Đăng ký");
            ...
            child: Text(
              "Đăng ký",
              style: TextStyle(
                color: isBusy ? Colors.grey : Colors.blue, // Màu nút khi bấm được hoặc không
              ),
            ),
          ),
          TextButton(
            onPressed: () {
              // Thực hiện hành động "Xem" (chắc chắn có thể bấm dù isBusy hay không)
              print("Xem");
              Navigator.of(context).pop();
            },
            child: Text("Xem"),
  ...
  void _moveToFirstPoint(List<LatLng> points) {
    if (points.isNotEmpty) {
      LatLng firstPoint = points.first; // Điểm đầu tiên
      _mapController.move(
          firstPoint, 18); // 14.0 là mức zoom, bạn có thể thay đổi
    }
  }
  ...
  void initState() {
    ...
    getLocation(widget.district, widget.name);
  }
