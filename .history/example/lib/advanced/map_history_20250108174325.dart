...
class MapHistory extends StatefulWidget {
  ...
  const MapHistory( {super.key, required this.idUser, required this.createTime, required this.endTime});
  @override
  State createState() => MapHistoryState();
}
class MapHistoryState extends State<MapHistory> with AutomaticKeepAliveClientMixin<MapHistory> {
  static const LOCATION_ARROW_IMAGE_PATH = "assets/images/markers/location-arrow-blue.png";
  List<LatLng> list = [];
Future<void> getLocation() async {
  final res = await ApiRequest.getLocation(widget.idUser, widget.createTime, widget.endTime);
  if (res.code == "200") {
    print("Thành công");
    // Chuyển đổi dữ liệu tọa độ thành List<LatLng>
     List data = res.data;
      List<LatLng> newList = data.map((location) {
        double longitude = double.parse(location["longitude"]);
        double latitude = double.parse(location["latitude"]);
        return LatLng(latitude, longitude);
      }).toList();

      // In danh sách tọa độ ra để kiểm tra
      newList.forEach((coordinate) {
        print(
            "Latitude: ${coordinate.latitude}, Longitude: ${coordinate.longitude}");
      });

      setState(() {
        // Cập nhật Marker với các điểm tọa độ
        _polygonGeofenceCursorMarkers = newList.asMap().entries.map((entry) {
          int index = entry.key;
          LatLng point = entry.value;

          // Nếu là điểm đầu tiên, sử dụng biểu tượng location màu đỏ
          if (index == 0) {
            return Marker(
              width: 40.0,
              height: 40.0,
              point: point,
              builder: (ctx) => Icon(
                Icons.location_on, // Biểu tượng location
                color: Colors.red,  // Màu đỏ cho điểm đầu tiên
                size: 30.0,
              ),
            );
          } else {
            // Tính toán góc xoay giữa điểm hiện tại và điểm tiếp theo
            LatLng prevPoint = newList[index - 1];
            double angle = _calculateBearing(prevPoint, point);
            ...
            return Marker(
              ...
              builder: (ctx) => Transform.rotate(
                angle: angle, // Xoay mũi tên theo góc tính được
                child: Icon(
                  Icons.navigation, // Biểu tượng mũi tên
                  color: Colors.blue, // Màu xanh cho các mũi tên
                  size: 30.0,
                ),
              ),
            );
        ...
        // Di chuyển bản đồ đến điểm cuối cùng
        _moveToFirstPoint(newList);
      });
...
// Hàm tính toán góc giữa hai điểm (bearing)
double _calculateBearing(LatLng start, LatLng end) {
  ...
  // Chuyển thành góc đối đỉnh (thêm 180 độ)
  bearing += math.pi;

  // Đảm bảo giá trị góc luôn nằm trong khoảng [0, 2π]
...
void _moveToFirstPoint(List<LatLng> points) {
  if (points.isNotEmpty) {
    LatLng firstPoint = points.last; // Điểm đầu tiên
    _mapController.move(firstPoint, 18); // 14.0 là mức zoom, bạn có thể thay đổi
  }
}
  @override
