...
Future<void> getLocation(List data) async {
  ...
  setState(() {
    _polygonGeofenceCursorMarkers = vehicles.map((vehicle) {
      ...
      return Marker(
        width: 100.0, // Kích thước để chứa cả icon và text
        height: 80.0, // Tăng chiều cao để phù hợp với bố cục
        point: point,
        builder: (ctx) {
          ValueNotifier<bool> isTapped = ValueNotifier(false); // Trạng thái màu icon

          return GestureDetector(
            onTapDown: (_) {
              isTapped.value = true; // Đổi trạng thái khi giữ
            },
            onTapUp: (_) {
              isTapped.value = false; // Trở lại trạng thái cũ khi thả
              _showMarkerDialog(context, vehicle);
            },
            onTapCancel: () {
              isTapped.value = false; // Đảm bảo reset nếu thao tác bị hủy
            },
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        shape: RoundedRectangleBorder(
          borderRadius: BorderRadius.circular(20.0), // Bo góc mềm mại hơn
        ),
        ...
        title: Row(
          children: [
            CircleAvatar(
              radius: 20,
              backgroundColor: isBusy ? Colors.red[100] : Colors.green[100],
              child: Icon(
                Icons.local_gas_station,
        ...
        actions: [
          ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(
              foregroundColor: isBusy ? Colors.grey : Colors.white, backgroundColor: isBusy ? Colors.grey[300] : Colors.blue,
              elevation: isBusy ? 0 : 3,
