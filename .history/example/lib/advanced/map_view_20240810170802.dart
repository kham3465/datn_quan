...
Future<void> createLocation(String longitude,String latitude,int idShip) async {
   final res = await ApiRequest.createLocation(longitude, latitude, idShip);
   if(res.code=="200"){
    print("thành công");
   }else{
    print("Thất bại");
...
class MapViewState extends State<MapView> with AutomaticKeepAliveClientMixin<MapView> {
  ...
  void _onLocation(bg.Location location) {
    ...
    LatLng ll = LatLng(location.coords.latitude, location.coords.longitude);
    createLocation(location.coords.longitude.toString(),location.coords.latitude.toString(),12);
