...
Future<void> getLocation(List data) async {
  ...
  // Danh sách lưu trạng thái "đã nhấn" cho từng Marker
  List<bool> markerTappedStatus =
      List.generate(vehicles.length, (index) => false);

  setState(() {
    _polygonGeofenceCursorMarkers = vehicles.asMap().entries.map((entry) {
      int index = entry.key;
      ElectricVehicle vehicle = entry.value;
      LatLng point = LatLng(
          double.parse(vehicle.latitude), double.parse(vehicle.longitude));

      return Marker(
        width: 100.0, // Tăng chiều rộng để chứa cả icon và text
        height: 70.0, // Tăng chiều cao để chứa cả icon và text
        point: point,
        builder: (ctx) {
          return GestureDetector(
            onTap: () {
              setState(() {
                // Đổi trạng thái của marker được nhấn
                markerTappedStatus[index] = !markerTappedStatus[index];
              });
            ...
            child: Column(
              ...
              children: [
                Icon(
                  ...
                  color: markerTappedStatus[index]
                      ? Colors.red // Đổi sang màu đỏ khi được bấm
